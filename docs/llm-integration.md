...
6. Use `browser_run_js` when the model needs page-local batching across many targets: custom selection, filtering, validation, dry-run planning, and optional execution in one or two calls instead of many small MCP actions. Return a JSON-serializable value and use `console.log` for streamed debug output.
